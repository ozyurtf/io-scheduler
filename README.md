...
- **Device drivers** are **responsible from initializing the IO device they control**. **Initialization** in here basically **means setting up the necessary configurations**, **registers**, and **parameters** that are **required for the IO device to function properly**. Initialization **ensures** that the **IO device** is **in ready condition** **before it can be used by the operationg system & applications**. 
- **Device drivers provide an abstraction layer between the operating system and the IO device**. And the **operating system commnuicates with the device driver using abstract read/write operations**. **Device driver receives these operations from the layer above (e.g., operating system) and translates these operations into commands that are specific to IO device**. The **device driver then executes these commands by communicating with the IO device controller**
...
_**Note**: **ROM** in here refers to **Read-Only Memory**. As we might understand from its name, the data **can be read from ROM** and it is **not easy to write data to it**. Also, ROM retains data after the power is turned off. Since the data in the ROM is permanent and not easily modifiable, the data that is essential for proper functioning of the system and that does not require frequent updates is generally stored in the ROM. **Although individual memory locations can be read arbitrarily**, an **entire block of memory needs to be erased at once before it can be reprogrammed**._ 

## Usage of EEPROMs
EEPROMs might be suitable to use in a systems that need to be programmed occasionally. They were used in the embedded processors in the past. Because the embedded systems did not require frequent updates, programming an EEPROM was not a time-critical event in the past. The number of times EEPROMs could be programmed was limited but this was not a big problem for embedded systems since they need to be programmed only small amount of times during their lifetime. 
