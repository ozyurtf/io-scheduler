...
Well the execution of the IO device driver that initiates an IO operation can be suspended until the IO operating finishes and the interrupt is handled. In other words, after initializing the necessary data structures and registers for the IO operation, the device driver can start an IO operation by sending commands to the IO device controller and block until the IO operation is completed. And in the meantime, the IO operation is handled by the IO device controller (not CPU), and it generates interrupt once the IO operation is done. And after interrupt is handled, the interrupt handler is invoked to handle the interrupt and perform post IO processing. And then the interrupt handler unblocks the IO device driver so that it can continue its execution.
