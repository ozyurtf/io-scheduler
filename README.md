...
When transfer is initiated, CPU programs the DMA controller by writing the necessary information into the control registers of the DMA controller. The DMA controller then accesses the system bus to read data from the source and writes it to the destination. The address registers and count registers are updated during the transfer. 

Once the transfer is finished, an interrupt is generated by the DMA controller to notify the CPU. Then the CPU performs any necessary operations
